...
;
segment	_cwMain align=16 public class="Main thread" use16
;
...
cwOpen:
	...
	;assume	ds:_cwMain
	;
	mov	d[TerminationHandler],cwClose
...
.KeepRaw:	sub	bx,ax		;Size program.
	...
	mov	b[ErrorNumber],1
	sys	IntXX
	test	w[es:edi+Real_Flags],1
	jnz	.9
	mov	w[ErrorNumber],0	;clear error number.
;
...
;
	or	w[ResourceTracking],-1	;Enable resource tracking.
;
;Run the main program.
;
	mov	edx,MainExec	;name to exec.
	mov	esi,80h
	mov	es,[PSPSegment]
	xor	cx,cx
...
%endif
	...
	;assume	ds:_cwMain
	;
	mov	w[ResourceTracking],0
	mov	w[mcbAllocations],0
	sti
...
.dd0:	;
	...
	cmp	b[EnableDebugDump],0	; if debug dump turned off, no screen i/o
	je	.NoError
...
;
	mov	di,[ExtensionList] ;list of interupt patches.
...
.p3:	popm	di,bp
.p2:	sub	di,2
	cmp	di,ExtensionList-2
	jnz	.p0
;
;Remove api exception patches.
;
	cmp	b[apiExcepPatched],0
	jz	.pe0
	mov	ax,cs
	push	ax
	mov	ax,.pe0
...
;
	mov	es,[apiDataSeg]
	;assume	es:_apiCode
	test	d[SystemFlags],1
	jz	.Use32
	mov	dx,w[es:OldIntSys]
	mov	cx,w[es:OldIntSys+2]
	jmp	.Use0
.Use32:	mov	edx,d[es:OldIntSys]
	mov	cx,w[es:OldIntSys+4]
.Use0: mov	bl,31h
	mov	ax,205h
	int	31h
	mov	d[es:cwIdentity],0
	mov	d[es:cwIdentity+4],0
	;
.noAPI:
	cmp	w[ProtectedType],2 ;DPMI?
	jz	.DPMI
...
Int21hExecPatch:
	...
	inc	b[cs:Int21hExecCount]

	popf
	pushf
	call	far [cs:OldInt21hExec]
...
;	and w[bp+2+2+2+2+2],1
	...
	dec	b[cs:Int21hExecCount]

	iret
.Old:
	popf
	jmp	far [cs:OldInt21hExec]
OldInt21hExec	dd 0
	;assume	ds:_cwMain
...
;Some global data.
;
RealPSPSegment	resw 1		;Real mode PSP segment.
RealENVSegment	resw 1		;Real mode environment segment.
...
;
TerminationHandler dd InitError,InitCS
UserTermRoutine dd	0
		dw	0
UserTermDump	dd	0	; dump location for register info
		dw	0
;
DPMIStateAddr	dd 0
		dw 0
...
CopyCount	dw 0
;
Int21Buffer	resb RealRegsStruc_size
Int10Buffer	resb RealRegsStruc_size
Int33Buffer	resb RealRegsStruc_size
;
...
Int21hExtension dw _Int21h,1		;segment start.
	dd Int21hEnd-Int21hStart ;segment length.
	dd Int21hOpen		;init code.
	resd 1
	dd Int21hClose		;remove code.
	resd 1
	dd 0			;code selector.
	dd 0			;data selector.
	dd 0			;installed flag.
...
Int10hExtension dw _Int10h,1		;segment start.
	dd Int10hEnd-Int10hStart ;segment length.
	dd Int10hOpen		;init code.
	resd 1
	dd Int10hClose		;remove code.
	resd 1
	dd 0			;code selector.
	dd 0			;data selector.
	dd 0			;installed flag.
...
Int33hExtension dw _Int33h,1		;segment start.
	dd Int33hEnd-Int33hStart ;segment length.
	dd Int33hOpen		;init code.
	resd 1
	dd Int33hClose		;remove code.
	resd 1
	dd 0			;code selector.
	dd 0			;data selector.
	dd 0			;installed flag.
...
;
segment	_cwStack align=16 stack class="main stack" use16
	times 2048 db 0
_cwStackEnd:
...
;
segment	_cwInit align=16 public class="init code" use16
	;assume cs:_cwInit, ds:_cwMain

dpmiSelBuffer	times 8 db 0
...
;
	cmp	w[ProtectedType],2 ;DPMI initialiseation?
	jz	.InitDPMI
;
...
;
	...
	mov	d[es:esi+eax*4],edx	;store this tables address.
	mov	esi,PageAliasLinear	;get alias table address.
	mov	d[es:esi+eax*4],edx	;setup in alias table as well.
	call	d[fCR3Flush]
...
;
	...
	mov	d[es:esi+eax*4],edx	;set physical address.
	call	d[fCR3Flush]
	mov	edi,LinearEntry
	shl	edi,12
	mov	ecx,4096/4
...
.6:	call	d[fPhysicalGetPage]	;try to allocate a page.
	...
	rep	stosb
	mov	edi,MDTLinear+4
	or	d[es:edi],-1		;Force VCPI values to not used.
	;
...
;
	...
	mov	d[es:esi+eax*4],0	;clear this pages details.
;
...
;
	test	d[SystemFlags],1 shl 14	;Dual mode?
	jnz	.Use16Bit23
	;
	test	d[SystemFlags],1
	jz	.Use32Bit23
...
;
	...
	mov	w[es:TerminationHandler+4],ax
	mov	ax,mDataSegment
...
;
	...
	test	d[SystemFlags],1
	jz	.Use32
	mov	w[es:OldIntSys],dx
	mov	w[es:OldIntSys+2],cx
	jmp	.Use0
.Use32:	mov	d[es:OldIntSys],edx
	mov	w[es:OldIntSys+4],cx
.Use0: mov	bl,31h
...
;
	...
	mov	w[es:EPSP_RealENV],ax
	mov	ax,offset Int21hExecCount
	mov	w[es:EPSP_ExecCount],ax
	mov	w[es:EPSP_ExecCount+2],ds
	mov	d[es:EPSP_Resource],edx ;Clear memory fields.
	mov	d[es:EPSP_INTMem],edx
	mov	d[es:EPSP_DPMIMem],edx
	mov	w[es:EPSP_Parent],es	;set parent PSP.
	mov	w[es:EPSP_Next],dx
	mov	d[es:EPSP_DTA],80h	;Use default PSP DTA.
	mov	w[es:EPSP_DTA+4],es
	mov	eax,16384-(mcbChunkLen+mcbLen)
	mov	d[es:EPSP_mcbMaxAlloc],eax
	mov	d[es:EPSP_mcbHead],edx
	mov	w[es:EPSP_SegBase],0
	mov	w[es:EPSP_SegSize],0
	mov	d[es:EPSP_LastPSP],0
	mov	d[es:EPSP_NextPSP],0
	mov	d[es:EPSP_Exports],0
	mov	d[es:EPSP_Imports],0
	mov	d[es:EPSP_Links],80000000h
	mov	d[es:EPSP_EntryCSEIP+4],0
	mov	w[es:EPSP_PSPSel],es
	mov	BasePSP,es
	mov	bx,es
	sys	GetSelDet32
	...
	sys	GetSel
	jc	InitError
	movzx	edx,w[es:PSP_HandlePtr+2]
	shl	edx,4
	movzx	ecx,w[es:PSP_Handles]
	movzx	eax,w[es:PSP_HandlePtr]
	add	edx,eax
	sys	SetSelDet32
	mov	dx,bx
.normal:	mov	w[es:PSP_HandlePtr+2],dx
	mov	w[es:PSP_HandlePtr],0
	pop	es
	assume ds:_cwInit
	pop	ds
;
...
;
	...
	push	es
	mov	es,PSPSegment
	mov	w[es:EPSP_TransProt],bx
	mov	ax,TransferReal
	mov	w[es:EPSP_TransReal],ax
	mov	eax,TransferSize
	mov	d[es:EPSP_TransSize],eax
	pop	es
	assume ds:_cwInit
	pop	ds
;
...
;
	...
	push	es
	mov	es,PSPSegment
	mov	d[es:EPSP_Exports],edi
	mov	es,RealSegment
	mov	ebp,es:[edi]
	add	edi,4
	add	d[es:edi],edx
	add	edi,4
.exp0: add	d[es:edi],edx
	mov	esi,es:[edi]
	mov	bx,CodeSegment
	cmp	w[es:esi+4],0
	jz	.exp1
...
.e0:	cmp	w[di],-1		;end of the list?
	...
	movzx	ecx,w[ds:bp+4]	;get patch size.
	...
	pushm	di,ds,es
	movzx	esi,w[ds:bp+0]	;get real mode segment base.
	shl	esi,4
	mov	ds,RealSegment
	mov	es,bx
	xor	edi,edi
	...
	rep	movsb
	popm	di,ds,es
	mov	w[ds:bp+32],-1	;flag installed.
	pushm	di,bp,ds
	mov	es,DataSegment
	mov	ds,ds:[bp+28]
...
.Use0: mov	bl,31h
	...
	;
.noAPI:
	cmp	w[cs:IProtectedType],2	;DPMI?
	assume ds:_cwInit
	jz	.DPMI
;
;Make RAW stuff addressable.
...
.NoDPMIForce:	test	ProtectedFlags,4
	jz	.1
	mov	w[ProtectedType],0 ;Use real mode.
	jmp	.3
.1:	test	ProtectedFlags,2
	jz	.2
	mov	w[ProtectedType],1 ;Use VCPI.
	jmp	.3
.2:	mov	w[ProtectedType],2 ;Use DPMI.
.3:	push	es
	mov	ax,_cwInit
	mov	es,ax
	assume es:_cwInit
...
ChkVCPI proc	near
	...
	jnz	.IsHandler
	cli
	mov	w[es:67h*4],offset .DummyIRET
	mov	w[es:(67h*4)+2],cs
...
VMMDrivPath1	times 128 db 0	;used by CAUSEWAY=SWAP:?:\??
VMMDrivPath2	times 128 db 0	;used by TEMP=
VMMDrivPath3	times 128 db 0	;used by TMP=
VMMDrivPath4	times 128 db 0	;used by current path.
VMMDrivPath5	times 128 db 0	;used by boot drive.
	db -1
;
DPMIErrRegsx	times RealRegsStruc_size db 0
;
...
;
segment	_cwEnd align=16 public class="end marker"
